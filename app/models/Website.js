...
class Website {
  ...
  static async findNew() {
    ...
    // On recupere simplement la liste des enregistrements
    const websites = result.rows;
    // Pour chaque websites, je veux creer un nouvel objet provenant de classe Website
    const websitesObj = [];
    // Pour chaque webiste contenu dans la bdd
  ...
  static async find(slug) {
    ...
    if (result.rowCount > 0) {
      return new Website(result.rows[0]); // on retourne une nouvelle instance
    }
    else {
      throw new Error('site non trouvée');
    }
  }
}
